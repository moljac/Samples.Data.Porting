...
public class UsbSendList {
	...
	public UsbSendList(Context context) {
		myContext = context;
	}
		...
		builer.setSingleChoiceItems(items, -1,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						// 发送对应内容
						// Send content
						UsbSend.SendContent(myContext, AllConstant[which]);
						// 此方法有4个参数
						// This method has 4 parameters
						// 第一个为Context,第二个是要发送的内容索引
						// The first is Context, second is the content index
						// to
						// send
						// 第三个是列表的名称（不是必须）
						// The third is a list of names (not required)
						// UsbSend.SendContent(myContext,AllConstant[which],"自定义列表名称");
						// 第四个是是否显示默认发送命令(默认是不显示，不是必须)
						// The fourth is whether to display the default send
						// command (the default is not displayed, not a
						// must)
						// UsbSend.SendContent(myContext,AllConstant[which],"自定义列表名称",true);
