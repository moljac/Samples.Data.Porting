...
public class UsbDataActivity extends Activity {
	...
	private void setDefaultChange() {
		EditText edt1 = ((EditText) findViewById(R.id.edt1));
		int max = UsbConnect.getNormalCharge(myActivity);
		edt1.setHint(String.valueOf(max));
		EditText edt2 = ((EditText) findViewById(R.id.edt2));
		int min = UsbConnect.getFastCharge(myActivity);
		edt2.setHint(String.valueOf(min));
		EditText edt3 = ((EditText) findViewById(R.id.edt3));
		int change = UsbConnect.getChargeWarn(myActivity);
		edt3.setHint(String.valueOf(change));
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(SendConstant.GetReadDataAction)) {
				...
				if (name.equals(myActivity.getString(R.string.gs_read_charge))) {
					...
					((TextView) findViewById(R.id.tvGetData)).append(name + ":"
							+ data);
				} 
