...
public class RemotePlayer implements ControllerType, SmcAvPlayer.EventListener, SmcAvPlayer.ResponseListener, SmcDeviceFinder.StatusListener {
    ...
    RemotePlayer(PlayerController controller, Uri contentUri,
                 Uri subtitlesUri, String mimeType, String deviceId, int deviceType) {
    ...
    private void playItem(){
        ...
        if (mSubtitlesUri != null) {
            item = new SmcItem(new SmcItem.LocalContent(mContentUri.getPath(),
                    mMimeType).setSubTitle(mSubtitlesUri.getPath()));
        } else {
            item = new SmcItem(new SmcItem.LocalContent(mContentUri.getPath(),mMimeType));
        }
     ...
     */
    @Override
    public void onDeviceChanged(SmcAvPlayer device, int state, int error) {
    ...
    @Override
    public void onRequestMediaInfo(SmcAvPlayer device, SmcAvPlayer.MediaInfo mediaInfo, int err) {
     ...
     */
    @Override
    public void onPlay(SmcAvPlayer device, SmcItem item, SmcAvPlayer.PlayInfo contentInfo, int error) {
        if (error != Smc.SUCCESS) {
            // Error has occurred.
            return;
        }
     ...
     */
    @Override
    public void onResume(SmcAvPlayer device, int error) {
        if (error != Smc.SUCCESS) {
            // Error has occurred.
            return;
        }
     ...
     */
    @Override
    public void onStop(SmcAvPlayer device, int error) {
        if (error != Smc.SUCCESS) {
            // Error has occurred.
            return;
        }
    ...
    /**
     * Provides common processing for state changing logic.
     *
    ...
    private void setState(PlayerState newState) {
        ...
        if (newState == mStateToGo) {
            // We have reached the state we were supposed to reach, so we can clear the flag.
            mStateToGo = null;
