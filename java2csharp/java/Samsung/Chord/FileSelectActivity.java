...
import com.samsung.android.sdk.chord.example.adapter.FileListAdapter;
...
public class FileSelectActivity extends Activity implements OnItemClickListener {
    ...
    public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
        ...
        if (file.isDirectory()) {
            if (file.canRead()) {
                ...
                new AlertDialog.Builder(this).setIcon(android.R.drawable.ic_lock_lock)
            ...
            Log.v(TAG, TAGClass + "fileSelected @@ file : " + file.getAbsolutePath());
        }
    }
    ...
    private void returnFilePath() {
        ...
        ArrayList<String> checkedFileList = mFilelistAdapter.getCheckedFileList();

        for (int i = 0; i < checkedFileList.size(); i++) {
            String mFilePath = mPwd_textView.getText() + "/" + checkedFileList.get(i);
    ...
    private final String TAG = "[Chord][Sample]";
