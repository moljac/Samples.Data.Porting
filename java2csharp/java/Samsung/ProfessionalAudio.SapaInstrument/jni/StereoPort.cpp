...
sample_t* StereoPort::getLeftPortBuffer(jack_nframes_t framesCount) noexcept
{
    return reinterpret_cast<sample_t *>(
            jack_port_get_buffer(left, framesCount)
            );
}

sample_t* StereoPort::getRightPortBuffer(jack_nframes_t framesCount) noexcept
{
    return reinterpret_cast<sample_t *>(
            jack_port_get_buffer(right, framesCount)
            );
}
